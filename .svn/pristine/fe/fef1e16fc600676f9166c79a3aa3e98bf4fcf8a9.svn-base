...
public class Configure {
	...
	// HTTPS证书的本地路径
	private static String certLocalPath = null;
	...
	//用户端实际ip
	public static String spBillCreateIP = null;
	...
	static {
		String conf_filename = "/properties/wxpay.properties";
		InputStream resourceAsStream = Configure.class.getResourceAsStream(conf_filename);
		Properties p = new Properties();
		try {
			log.i("加载wxpay配置文件【" + conf_filename + "】.................................");
			p.load(resourceAsStream);
			log.i(p.toString());
			key = p.getProperty("wx.key");
			secret = p.getProperty("wx.secret");
			appID = p.getProperty("wx.appID");
			mchID = p.getProperty("wx.mchID");
			subMchID = p.getProperty("wx.subMchID");

			// 证书存放地方
			certLocalPath = p.getProperty("wx.certLocalPath");
			if (certLocalPath == null || "".equals(certLocalPath)) {
				throw new IllegalArgumentException();
			}
			if (!certLocalPath.startsWith("/")) {
				certLocalPath = "/" + certLocalPath;
			}
			certLocalPath = Configure.class.getResource(certLocalPath).getPath();
			log.i("证书certLocalPath【" + certLocalPath + "】...");

			certPassword = p.getProperty("wx.certPassword");
			ip = p.getProperty("wx.ip");
			notify_url = p.getProperty("wx.notify_url");
			deviceInfo = p.getProperty("wx.deviceInfo");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
